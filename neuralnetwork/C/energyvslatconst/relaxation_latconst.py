...
def equilibrate_graphene_dropout(potential, ainit, nx=1, ny=1):
    """Get an ensemble of lattice constant and cohesive energy from dropout ensemble."""
    # Generate graphene sheet
    cell, pos = generate_unit_cell(ainit, unit="lattice", zshift=0.0)
    natoms = len(pos)
    ...
    a0_list = []
    e0_list = []
    for ii in tqdm(range(100)):  # There are 100 dropout members
        # Write input file
        loader = jinja2.FileSystemLoader(WORK_DIR)
        environment = jinja2.Environment(loader=loader)
        template = environment.get_template("relaxation_dropout.tpl")
        content = template.render(
            potential=potential,
            active_id=ii + 1,
            inita=ainit,
