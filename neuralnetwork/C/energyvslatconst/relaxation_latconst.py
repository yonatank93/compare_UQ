...
try:
    from .generate_cell import generate_unit_cell_graphene, generate_unit_cell_graphite
except ImportError:
    from generate_cell import generate_unit_cell_graphene, generate_unit_cell_graphite
...
template_file_graphite = "relaxation_graphite.tpl"
...
def equilibrate_graphene(potential, ainit, nx=1, ny=1, active_member_id=0, debug=False):
    ...
    cell, pos = generate_unit_cell_graphene(ainit, unit="lattice", zshift=0.0)
    natoms = len(pos)
...
def equilibrate_graphite(potential, ainit, cinit, active_member_id=0, debug=False):
    """Relax a graphite structure and get the equilibrium lattice constant and cohesive
    energy.
    ...
    potential: str
        KIM ID for the potential to use.
    ainit, cinit: float
        Initial guess of in-plane and out-of-plane lattice constants, respectively,
        in angstrom.
    active_member_id: int
        Integer number that sets the active member in the ensemble. 0 means to not use
        dropout, -1 means to take the mean across all ensemble, and 1-100 correspond to
    ...
    -------
    a0: float
        Equilibrium in-plane lattice constant a in angstrom.
    c0: float
        Equilibrium out-of-plane lattice constant a in angstrom.
    e0: float
        Cohesive energy in eV.
    """
    # Generate graphite sheet
    cell, pos = generate_unit_cell_graphite(ainit, cinit, unit="lattice")
    natoms = len(pos)
    ...
    loader = jinja2.FileSystemLoader(WORK_DIR)
    environment = jinja2.Environment(loader=loader)
    template = environment.get_template(template_file_graphite)
    content = template.render(
        potential=potential,
        active_id=active_member_id,
        inita=ainit,
    ...
    lmp = lammps.lammps(cmdargs=["-screen", os.devnull, "-nocite"])
    lmp.commands_string(content)
    a0 = lmp.extract_variable("length")  # Equilibrium in-plane lattice constant
    c0 = lmp.extract_variable("height")  # Equilibrium out-of-plane lattice constant
    e0 = lmp.extract_variable("ecoh")  # Equilibrium cohesive energy
    lmp.close()

    if debug:
        return a0, c0, e0, content
    else:
        return a0, c0, e0
