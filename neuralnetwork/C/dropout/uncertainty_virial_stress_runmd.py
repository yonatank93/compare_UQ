...
kim		interactions C
kim		param set active_member_id 1 {{ idx }}
...
def write_lammps_command(modelname, a, idx, dumppath):
    ...
    lammps_command = template.render(
        modelname=modelname, latparam=a, idx=idx, path=dumppath
    )
    return lammps_command


def run_one_latparam(
    modelname, a, idx, dumppath, cmdargs=["-screen", os.devnull, "-nocite"]
):
    ...
    lammps_command = write_lammps_command(modelname, a, idx, dumppath)
    # Run lammps
...
def run_md_one_latparam(a, modelname, idx, savepath):
    ...
    if not outfile.exists():
        run_one_latparam(modelname, a, idx + 1, path, cmdargs=["-screen", str(outfile)])
    # Post-process
