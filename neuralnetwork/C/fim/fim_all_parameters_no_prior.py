...
# Directories
WORK_DIR = Path(__file__).absolute().parentk
...
RES_DIR = (
    WORK_DIR / "results" / f"{partition}_partition_{suffix}" / "all_parameters_no_prior"
)
...
if param_samples_file.exists():
    ...
    cov = 0.5 * (cov + cov.T)  # Make sure covariance is symmetric
    ...
    try:
        ...
        # Let's try another way to generate the samples
        L = np.linalg.cholesky(cov)
        normal_samples = np.random.randn(nparams, nsamples)
        samples_zero_mean = L @ normal_samples
        param_samples = p0 + samples_zero_mean.T
