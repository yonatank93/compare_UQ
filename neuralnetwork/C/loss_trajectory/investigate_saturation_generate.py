...
model_params_file = (
    TRAIN_RES_DIR / "models" / f"model_epoch{epoch_list[args.set_idx]}.pkl"
)
model.load(model_params_file)

# Remove all the dropout layers and the last linear layer
model.layers = [layer for layer in model.layers if not isinstance(layer, nn.Dropout)]
model.layers.pop()
