...
if ensemble_loss_file.exists():
    ensemble_loss = np.loadtxt(ensemble_loss_file)
else:
    ensemble_loss = np.zeros(100)

for ii in tqdm(range(100)):
    if ensemble_loss[ii] == 0.0:
        # Model
        model_name = f"DUNN_C_losstraj_{ii:03d}"
        MODEL_DIR = RES_DIR / f"{ii:03d}" / model_name
        # Install the model
        # subprocess.run(
        #     f"kim-api-collections-management remove --force {model_name}", shell=True
        # )
        subprocess.run(
            f"kim-api-collections-management install user {MODEL_DIR}", shell=True
        )
        ...
        # Turn off dropout
        calc.model.kim_model.set_parameter(1, 0, 0)  # pidx, array_idx, update_value
        ...
        ensemble_loss[ii] = 0.5 * np.linalg.norm(residual) ** 2
